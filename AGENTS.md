...
- 将当前未提交的代码全部提交到 Git。
- Git 提交信息统一使用 `英文前缀: 中文描述`，例如 `refactor: 添加 provider 适配层并接入画布生成`。
- 多行 Git 提交信息必须使用 `subject`、空行、`body` 的结构；`subject` 保持一行摘要，详细说明放到 `body`，并用段落、标题和列表排版，不要把多段内容挤成一整段。
- 在 Windows / PowerShell 中写多行提交信息时，优先写入 UTF-8 message 文件后用 `git commit -F <file>` 或 `git commit --amend -F <file>` 提交，避免管道或命令行参数导致中文编码异常。
